...
export class TokenVerifier {
  constructor({
    ...
    // ENFORCE asymmetric only for platform auth - never allow HS*
    const ASYMMETRIC_ONLY = [
      'RS256',
      'RS384',
      'RS512',
      'ES256',
    ...
    if (!ASYMMETRIC_ONLY.includes(algorithm)) {
      throw new Error(
        `[TokenVerifier] For platform auth, only asymmetric allowed: ${algorithm}`
      );
    }
    ...
    if (!options.issuer || !options.audience) {
      throw new Error(
        '[TokenVerifier] issuer and audience are mandatory for high-integrity systems.'
      );
    }
  ...
  async _getKey(header) {
    ...
    if (typeof this.keyResolver === 'function') {
      ...
      if (!key) {
        throw new Error(`Unable to resolve key for kid: ${header.kid}`);
      }
  ...
  async verify(token) {
    ...
    try {
      ...
      if (typeof this.keyResolver === 'function') {
        const unverified = this.jwtProvider.decode(cleanToken, {
          complete: true,
        });
      ...
      const decoded = await this.jwtProvider.verify(
        cleanToken,
        key,
        this.options
      );
      ...
      if (this.revocationStore) {
        ...
        if (isRevoked) {
          this.logger?.warn(
            `[TokenVerifier] Revoked token: user=${decoded.sub} jti=${decoded.jti}`
          );
      ...
      this.logger?.error(
        `[TokenVerifier] Verify failed: ${err.message}`,
        {
          alg: this.algorithm,
          error: err.name,
        }
