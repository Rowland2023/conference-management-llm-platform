...
const holdFundsSchema = z.object({
  body: z.object({
    ...
    amount: z
      ...
      .trim()
      .regex(/^[1-9]\d*$/, 'Amount must be a positive integer string in minor units (e.g., 500 for $5.00)'),
    ...
    expiresInSeconds: z
      .number({ invalid_type_error: 'expiresInSeconds must be a number' })
      .int('expiresInSeconds must be an integer')
      .positive('expiresInSeconds must be positive')
      .max(2592000, 'Hold duration cannot exceed 30 days (2,592,000 seconds)')
      .optional(),
  }),
});
